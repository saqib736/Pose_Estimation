...
def bbox_to_rle(bbox, image_height, image_width):
    """
    Convert a bounding box to RLE.
    ...
    Returns:
    - rle: RLE representation of the mask.
    """
    mask = np.zeros((image_height, image_width), dtype=np.uint8)
    x_min, y_min, width, height = [int(val) for val in bbox]  # Ensure the bbox coordinates are integers
            ...
            if self.with_mask:
                ...
                for p_id in list(persons_ids):
                    ...
                    if person_meta["iscrowd"]:
                        is_crowd=True
                        # Use bbox to create mask RLE when iscrowd is true or segmentation is missing
                        bbox_rle = bbox_to_rle(person_meta["bbox"], int(image_info['height']), int(image_info['width']))
                        masks.append(bbox_rle)
                        continue
